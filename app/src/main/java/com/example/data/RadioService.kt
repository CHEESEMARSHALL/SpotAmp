...
class RadioService {
    ...
    fun generate(request: RadioRequest, tracks: List<CachedTrack>, history: List<RecentTrack> = emptyList()): List<TrackItem> {
        ...
        val candidates = when (request.type) {
            RadioType.ARTIST_RADIO -> tracks.filter { request.seedArtist != null && it.artist.equals(request.seedArtist, true) }
            RadioType.ALBUM_RADIO -> tracks.filter { request.seedAlbum != null && it.album.equals(request.seedAlbum, true) }
            ...
            RadioType.DECADE_RADIO, RadioType.TIME_TRAVEL -> tracks.filter { request.decadeStart != null && it.year?.let { year -> year in request.decadeStart..(request.decadeStart + 9) } == true }
        ...
        for (track in sorted) {
            if (request.type != RadioType.ARTIST_RADIO && artistCounts.getOrDefault(track.artist.lowercase(Locale.ROOT), 0) >= request.maxTracksPerArtist) continue
        ...
        return result.map { it.toTrackItem() }
    }
}
